"""
Dump a HAR file from a running mitmdump instance and also
blacklist any URL passed with the given HTTP status code

This inline script can be used to dump flows as HAR files.

example command line invocation:
mitmdump -s ./har_dump.py --set hardump=./dump.har --set partial_url=/some/partial/url/
--set status_code=403
...
# Response counter for har dump frequency
RESPONSES = 0
...
def done():
    ...
    if ctx.options.hardump:
        ...
        if os.path.isfile(os.path.expanduser(ctx.options.hardump)):
            os.remove(os.path.expanduser(ctx.options.hardump))
        if ctx.options.hardump == '-':
...
def ignore_responses(flow):
    ...
    if not all(s not in mime_type for s in IGNORE_CONTENT_TYPES):
        mitmproxy.ctx.log(
            "Not saving response to HAR (matches: IGNORE_CONTENT_TYPE)")
        return True
    if not all(s not in flow.request.path for s in IGNORE_PARTIAL_URLS):
        mitmproxy.ctx.log(
            "Not saving response to HAR (matches: IGNORE_PARTIAL_URLS)")
        return True
    return False
